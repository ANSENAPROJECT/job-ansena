...
const InActiveJob = ({
  ...
  const inactiveJob = useSelector((state) => state.viewjob.inactiveJobGroup);
  ...
  const submitDeactive = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.data.inactive.length === 0) {
          setCollapse(true);
        }
      })
      .catch(({response}) => {
  ...
  return (
    <>
      <TouchableOpacity
        disabled={inactiveJob.length === 0 ? true : false}
        style={styles.container}
        onPress={() => {
          setCollapse(!collapse);
        }}>
        <Text style={styles.titleTxt}>Inactive Job Groups</Text>
...
export default connect(null, mapDispatchToProps)(InActiveJob);
